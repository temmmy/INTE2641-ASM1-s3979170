...
export class MerkleTreeDemo {
    ...
    public constructMerkleTree(dataItems: string[]): string {
        ...
        let currentLevel: MerkleNode[] = dataItems.map((data, index) => {
            ...
            console.log(`   Leaf ${index + 1}: "${data}" → ${hash.substring(0, 16)}...`);
            return {
                hash,
                data,
        ...
        while (currentLevel.length > 1) {
            ...
            for (let i = 0; i < currentLevel.length; i += 2) {
                ...
                if (i + 1 >= currentLevel.length) {
                    ...
                    console.log(`   ⚠️  Odd number of nodes, duplicating: ${leftNode.hash.substring(0, 16)}...`);
                } else {
                ...
                console.log(`   Parent ${Math.floor(i / 2) + 1}: ${leftNode.hash.substring(0, 8)}... + ${rightNode.hash.substring(0, 8)}... → ${combinedHash.substring(0, 16)}...`);
        ...
        console.log(`📏 Tree Height: ${this.calculateTreeHeight(dataItems.length)} levels`);
    ...
    public generateMerkleProof(dataItem: string): MerkleProof {
        ...
        if (!this.tree) {
            throw new Error('Merkle tree must be constructed before generating proofs');
        }
        ...
        for (let i = path.length - 1; i > 0; i--) {
            ...
            if (siblingNode) {
                ...
                console.log(`   Level ${proofStep.level}: ${proofStep.position.toUpperCase()} sibling = ${proofStep.hash.substring(0, 16)}...`);
            }
        }
     ...
     */
    private findPathToLeaf(node: MerkleNode, targetData: string, path: MerkleNode[] = []): MerkleNode[] | null {
        ...
        if (node.right) {
            const rightPath = this.findPathToLeaf(node.right, targetData, currentPath);
    ...
    private displayProofStructure(proof: MerkleProof): void {
        ...
        proof.steps.forEach((step, index) => {
            ...
            console.log(`Step ${index + 1} (Level ${step.level}): ${arrow} ${step.hash.substring(0, 16)}... (${step.position})`);
        });
    ...
    public verifyMerkleProof(proof: MerkleProof, merkleRoot: string): boolean {
        ...
        try {
            ...
            console.log(`🏁 Starting with data hash: ${currentHash.substring(0, 16)}...`);
            ...
            proof.steps.forEach((step, index) => {
                ...
                console.log(`   Sibling hash: ${step.hash.substring(0, 16)}... (${step.position})`);
                ...
                if (step.position === 'left') {
                    ...
                    console.log(`   Combined: LEFT(${step.hash.substring(0, 8)}...) + RIGHT(${proof.dataHash.substring(0, 8)}...)`);
                } else {
                    // Current hash on the left, sibling on the right
                    currentHash = this.combineHashes(currentHash, step.hash);
                    console.log(`   Combined: LEFT(${proof.dataHash.substring(0, 8)}...) + RIGHT(${step.hash.substring(0, 8)}...)`);
                }
            ...
            if (isValid) {
                ...
                console.log(`   Data item "${proof.dataItem}" is confirmed to be in the Merkle tree`);
                console.log(`   The proof demonstrates inclusion without revealing other data items`);
            } else {
                console.log('\n❌ FAILURE: Proof verification failed!');
                console.log(`   Either the data item is not in the tree, or the proof is invalid`);
                console.log(`   Hash mismatch indicates potential tampering or incorrect proof`);
            }
     ...
     */
    private explainVerificationProcess(proof: MerkleProof, isValid: boolean): void {
        console.log('\n🎓 EDUCATIONAL ANALYSIS:');
        console.log('========================');
        
        console.log(`📈 Efficiency Benefits:`);
        console.log(`   • Proof size: ${this.calculateProofSize(proof)} bytes vs full tree`);
        console.log(`   • Verification time: O(log n) vs O(n) for full verification`);
        console.log(`   • Network bandwidth: Only ${proof.steps.length} hashes needed`);
        
        console.log(`\n🔒 Security Properties:`);
        if (isValid) {
            console.log(`   • Cryptographic guarantee: Data item is definitely in the tree`);
            ...
            console.log(`   • Trust model: Verification doesn't require trusted parties`);
        }

        console.log(`\n⚡ SPV Applications:`);
        console.log(`   • Lightweight clients can verify transactions without full blockchain`);
        console.log(`   • Mobile wallets use Merkle proofs for transaction confirmation`);
        console.log(`   • Scalability: Millions of transactions verified with small proofs`);
    }
    ...
    public runCompleteDemo(dataItems: string[]): void {
        ...
        try {
            ...
            if (dataItems.length > 1) {
                ...
                console.log(`   • Original proof: ${isValid ? '✅ Valid' : '❌ Invalid'}`);
                console.log(`   • Second item proof: ${secondValid ? '✅ Valid' : '❌ Invalid'}`);
                console.log(`   • Invalid proof test: ${invalidResult ? '⚠️ Unexpected success' : '✅ Correctly rejected'}`);
            }

            console.log('\n🎯 DEMONSTRATION COMPLETE');
            console.log('All Merkle tree operations have been successfully demonstrated.');
            console.log('See the written analysis for detailed explanations of these concepts.\n');
...
export class MerkleTreeCLI {
    ...
    public async run(): Promise<void> {
        ...
        console.log('• Review the written analysis (Problem 2B) for theoretical explanations');
        console.log('• Try running with different numbers of transactions (odd/even)');
        console.log('• Experiment with different data items to see proof variations');
        console.log('• Consider how this enables SPV clients in real blockchain systems');
    }
}
