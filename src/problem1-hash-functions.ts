    ...
    public demonstrateAvalancheEffect(originalInput: string): void {
        ...
        const modifications = [
            {
                ...
                modify: (str: string) => str.slice(0, -1) + String.fromCharCode(str.charCodeAt(str.length - 1) + 1)
            },
            {
                description: 'Single bit flip (first character)',
                modify: (str: string) => String.fromCharCode(str.charCodeAt(0) ^ 1) + str.slice(1)
            },
            {
        ...
        modifications.forEach((mod, index) => {
            try {
                ...
                const charHammingDistance = this.calculateHammingDistance(originalHash, modifiedHash);
                const bitHammingDistance = this.calculateBitHammingDistance(originalHash, modifiedHash);
                const totalBits = originalHash.length * 4; // Each hex char = 4 bits
                const changePercentage = ((bitHammingDistance / totalBits) * 100).toFixed(2);
                ...
                console.log(`   
   📏 Difference Analysis:`);
                console.log(`   • Character differences: ${charHammingDistance}/${originalHash.length} (${((charHammingDistance/originalHash.length)*100).toFixed(1)}%)`);
                console.log(`   • Bit differences: ${bitHammingDistance}/${totalBits} (${changePercentage}%)`);
                console.log(`   • Input change: ${this.getInputChangeDescription(originalInput, modifiedInput)}\n`);
        ...
        console.log('• Even minimal input changes (1 bit, 1 character) cause massive hash changes');
        console.log('• Typically ~50% of bits change, demonstrating strong avalanche properties');
        console.log('• This property is crucial for blockchain integrity and security\n');
    }
     ...
     */
    private getInputChangeDescription(original: string, modified: string): string {
    ...
    public demonstratePreImageResistance(
        ...
        console.log(`Search Method: ${useRandomSearch ? 'Random string generation' : 'Sequential enumeration'}`);
        ...
        for (attempts = 1; attempts <= maxAttempts; attempts++) {
            ...
            if (attempts % reportInterval === 0) {
                ...
                console.log(`   Progress: ${progress}% (${attempts.toLocaleString()} attempts, ${elapsed}s elapsed)`);
            }
        }
        ...
        if (found) {
            console.log(`🎯 SUCCESS: Found pre-image after ${attempts.toLocaleString()} attempts!`);
            console.log(`   Found Input: "${foundInput}"`);
            console.log(`   Found Hash:  ${this.computeHash(foundInput)}`);
            console.log(`   ⚠️  This is extremely unlikely for secure hash functions!`);
        } else {
            console.log(`❌ FAILED: No pre-image found after ${attempts.toLocaleString()} attempts`);
            console.log(`   This demonstrates the pre-image resistance of ${this.hashAlgorithm.toUpperCase()}`);
        }
        
        console.log(`\n⏱️  Performance Metrics:`);
        console.log(`   • Total time: ${totalTime} seconds`);
        console.log(`   • Attempts per second: ${attemptsPerSecond}`);
        console.log(`   • Search space explored: ~${this.calculateSearchSpaceExplored(attempts, targetString.length)}`);
     ...
     */
    private generateSequentialString(attempt: number, baseLength: number): string {
     ...
     */
    private calculateSearchSpaceExplored(attempts: number, stringLength: number): string {
     ...
     */
    private explainPreImageResistance(attempts: number, found: boolean, hashLength: number): void {
        ...
        console.log(`• Hash output space: 2^${hashBits} = ${Math.pow(2, hashBits).toExponential(2)} possible values`);
        console.log(`• Expected attempts for pre-image: ~2^${hashBits - 1} = ${theoreticalAttempts.toExponential(2)}`);
        console.log(`• Our attempts: ${attempts.toLocaleString()}`);
        console.log(`• Probability of success: ~${(attempts / Math.pow(2, hashBits) * 100).toExponential(2)}%`);
        
        if (!found) {
            console.log(`\n✅ This demonstrates why ${this.hashAlgorithm.toUpperCase()} is considered pre-image resistant:`);
            console.log(`   • Finding a pre-image requires astronomical computational effort`);
            console.log(`   • Current technology cannot feasibly break this security property`);
     ...
     */
    public runCompleteDemo(userInput: string, preImageAttempts: number = 100000): void {
        ...
        console.log('Both avalanche effect and pre-image resistance have been demonstrated.');
        console.log('See the written analysis for detailed explanations of these properties.\n');
    }
}
...
export class HashFunctionCLI {
    ...
    public async run(): Promise<void> {
        ...
        console.log('(In interactive mode, you would be prompted to enter your own string)\n');
        
        // Run the complete demonstration
        this.demo.runCompleteDemo(defaultInput, 50000);
        
        console.log('💡 NEXT STEPS:');
        console.log('• Review the written analysis (Problem 1B) for theoretical explanations');
        console.log('• Try running with different input strings to see varied results');
        console.log('• Increase the pre-image attempt count for more thorough testing');
    }
}
